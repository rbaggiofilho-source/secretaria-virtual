...
create index if not exists memories_user_kind_idx
  on public.memories (user_wa, kind);

create index if not exists memories_user_obra_idx
  on public.memories (user_wa, obra);
...
create index if not exists conversations_user_created_idx
  on public.conversations (user_wa, created_at desc);
